<p align="center">
  ...
  <img src="images/plugin-logo.png" alt="Area Rewind" width="180" />
</p>
...
- **Backup Preview:** Visualise what a backup looked like using a particle-overlay of all changed blocks â€” supports any Bukkit particle type
...
- **Custom Restore Speed:** Set a per-area restore rate (1â€“10 000 blocks per tick) or let the system dynamically choose an optimal batch size
...
5. Start the server â€” the plugin generates `plugins/AreaRewind/` containing `config.yml` and area data files
...
| `/rewind rollback <area> <time>` | Roll back to the closest backup at a given time offset â€” valid units: `m` (minutes), `h` (hours), `d` (days), `w` (weeks); example: `2h`, `30m`, `1d` |
...
| `list` | â€” | Show your current personal settings |
...
**Aliases:** `/ar`, `/arearewind`, `/protect` â€” **Selection tool:** Wooden hoe
